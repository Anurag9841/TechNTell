...
export const postOrder = (productId, quantity, totalPrice) => (dispatch) => {
  ...
  return fetch(baseUrl + "products/" + productId, {
    ...
    .then((order) => dispatch(addOrder(order)))
    .catch((err) => {
      console.log("Post Order", err.message);
...
export const deleteOrder = (orderId, orderDetailsId) => (dispatch) => {
  ...
  return fetch(baseUrl + "orders/" + orderId + "/" + orderDetailsId, {
    ...
    .then((respone) => respone.json())
    .catch((err) => {
      console.log("Delete Order ", err.message);
    });
};

export const updateOrder =
  (orderId, orderDetailsId, quantity, to_be_suppliedDate, shippedDate) =>
  (dispatch) => {
    const updatedOrder = {
      quantity: quantity,
    ...
    return fetch(baseUrl + "orders/" + orderId + orderDetailsId, {
      ...
      .then((order) => dispatch(addOrder(order)))
      .catch((err) => {
        console.log("Put Order", err.message);
...
export const postCollection =
  ...
  (dispatch) => {
    console.log("pcollection fired")
    const newCollection = {
    };
    if(collectionName != null)
      {newCollection.collectionName= collectionName}
      else{
        newCollection.collectionsForColumnsChosen = [];
      }
      ...
      if(prodChosen != null)
      {newCollection.prodChosen= prodChosen}

      
      newCollection.collectionType= collectionType

      if(columnsComboName!=null){
      newCollection.columnsComboName= columnsComboName}

      if(columnsComboName!=null){

      newCollection.columnsComboDescription= columnsComboDescription}

      if(columnsChosen!=null){
      newCollection.columnsChosen= columnsChosen}

    console.log("newCollection", newCollection);

    return fetch(baseUrl + "users/addCollection", {
      method: "POST",
...
export const getCollections = () => (dispatch) => {
  dispatch(getCollectionsLoading());

  return fetch(baseUrl + "users/currentUser/collections", {
    headers: {
      Authorization: localStorage.getItem("token"),
    },
  })
    ...
    .then((response) => response.json())
    .then((collections) => dispatch(getCollectionsSuccess(collections)))
    .catch((error) => {
      dispatch(getCollectionsFailure(error.message));
    });
};

const getCollectionsLoading = () => (
  
  {
  
  type: ActionTypes.PC_AND_CUSTOM_COLLECTIONS_LOADING,
});
const getCollectionsSuccess = (collections) => ({
  type: ActionTypes.PC_AND_CUSTOM_COLLECTIONS_SUCCESS,
  payload: collections,
});
const getCollectionsFailure = (errMess) => ({
  type: ActionTypes.PC_AND_CUSTOM_COLLECTIONS_FAILURE,
  payload: errMess,
});
