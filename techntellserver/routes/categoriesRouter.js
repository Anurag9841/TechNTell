...
var {Categories, ComponentCategories} = require("../models/categoryModel");
...
CategoriesRouter.route("/")
  ...
  .post(isAdmin, (req, res, next) => {
    ComponentCategories.findOne({ categoryName: req.body.categoryName })
      .then(
        (categ) => {
          if (categ == null) {
            ...
            res.statusCode = 404;
            var err = new Error("The category exists in Components Collection");
            next(err);
          }
        },
        (err) => next(err)
...
CategoriesRouter.route("/:categId").get((req, res, next) => {
        ...
        (category) => {
          if (category != null) {
            ...
            for (i = 0; i < category.products.length; i++) {
              Products.findOneAndDelete({ _id: category.products[i] }).then(
                (product) => {
                  if (product != null) {
                    res.json(product);
                  } else {
                    res.statusCode = 404;
                    var err = new Error(
                      "Error: no product available to delete"
                    );
                    next(err);
                  }
                }
            ...
            console.log(category.products.length);
            category
              .save(category.products.splice(0, category.products.length))
              .then(
                (category) => {
                  res.statusCode = 200;
                  res.setHeader("content-type", "application/json");
...
CategoriesRouter.route("/:categId").delete((req, res, next) => {
  Categories.findOne({ categoryName: req.params.categId })
    .then(
      (category) => {
        if (category != null) {
        ...
        (category) => {
          if (category != null) {
            ...
            if (index > -1) {
              ...
              res.statusCode = 404;
              var err = new Error(
                "Error: no such product id in the category " +
